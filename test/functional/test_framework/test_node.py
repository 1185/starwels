...
class TestNode():
    ...
    def __init__(self, i, datadir, *, rpchost, timewait, starwelsd, starwels_cli, mocktime, coverage_dir, extra_conf=None, extra_args=None, use_cli=False):
        self.index = i
        ...
        self.cli = TestNodeCLI(starwels_cli, self.datadir)
    ...
    def start(self, extra_args=None, *, stdout=None, stderr=None, **kwargs):
        ...
        # Add a new stdout and stderr file each time starwelsd is started
        if stderr is None:
    ...
    def wait_for_rpc_connection(self):
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            if self.process.poll() is not None:
                raise FailedToStartError(self._node_msg(
                    'starwelsd exited with status {} during initialization'.format(self.process.returncode)))
            try:
        ...
        self._raise_assertion_error("Unable to connect to starwelsd")
    ...
    def assert_start_raises_init_error(self, extra_args=None, expected_msg=None, match=ErrorMatch.FULL_TEXT, *args, **kwargs):
        ...
        extra_args: extra arguments to pass through to starwelsd
        expected_msg: regex that stderr should match when starwelsd fails

        Will throw if starwelsd starts without an error.
        Will throw if an expected_msg is provided and it does not match starwelsd's stdout."""
        with tempfile.NamedTemporaryFile(dir=self.stderr_dir, delete=False) as log_stderr, \
            ...
            try:
                ...
                self.log.debug('starwelsd failed to start: %s', e)
                self.running = False
                self.process = None
