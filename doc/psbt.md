# PSBT Howto for Starwels

Since Starwels 0.17, an RPC interface exists for Partially Signed Starwels
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/starwels/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Starwels transactions that are not fully signed
...
Overall, the construction of a fully signed Starwels transaction goes through the
following steps:
...
- An **Extractor** produces a valid Starwels transaction (in network format)
...
#### Multisig with multiple Starwels instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
Starwels. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `starwels-cli` is used.
...
Later, when *V* USDH has been received on *Amulti*, and Bob and Carol want to
