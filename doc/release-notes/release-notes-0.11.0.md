Starwels version 0.11.0 is now available from:

  <https://bitcoin.org/bin/bitcoin-0.11.0/>
...
block index and UTXO databases. The minimum was chosen so that Starwels will 
...
For an example script see [starwels-submittx](https://github.com/laanwj/bitcoin-submittx).
...
- #5521 `8e4578a` Reject non-final txs even in ai/regtest
...
- #5334 `cf87536` libstarwelsconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new starwels-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only StarwelsAmountField
- #5753 `0f44672` Add starwels logo to about screen
...
- #5809 `46bfbe7` Add starwels-cli man page
...
- #5849 `41259ca` contrib/init/starwelsd.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Starwels-Qt.app to Starwels.app
- #6168 `b3024f0` contrib/linearize: Support linearization of ai blocks
- #6098 `7708fcd` Update Windows resource files (and add one for starwels-tx)
