Starwels version 0.14.0 is now available from:

  <https://bitcoin.org/bin/bitcoin-0.14.0/>
...
Starwels has supported automatically pruning the blockchain since 0.11. Pruning
...
`"ai"`	   | `getblockchaininfo()["chain"] == "test"`
...
now ZMQ can be used on Windows. Please see [this document](https://github.com/starwels/starwels/blob/master/doc/zmq.md) for
...
When Starwels is out-of-sync on startup, a semi-transparent information
...
Commands sent over the JSON-RPC interface and through the `starwels-cli` binary
can now use named arguments. This follows the [JSON-RPC specification](http://www.jsonrpc.org/specification)
for passing parameters by-name with an object.

`starwels-cli` has been updated to support this by parsing `name=value` arguments
...
The Alert System was [disabled and deprecated](https://bitcoin.org/en/alert/2016-11-01-alert-retirement) in Starwels 0.12.1 and removed in 0.13.0. 
...
----------------------
 ...
 - RPC command `getmininginfo` loses the "ai" field in favor of the more
 ...
 - A new RPC command `getmemoryinfo` has been added which will return information
   about the memory usage of Starwels. This was added in conjunction with
...
- #8722 `bb843ad` starwels-cli: More detailed error reporting (laanwj)
...
- #8608 `7e9ab95` Install manpages via make install, also add some autogenerated manpages (nomnommai)
...
- #8568 `078900d` new var `DIST_CONTRIB` adds useful things for packagers from contrib (nomnommai)
...
- #8830 `ef0801b` test: Add option to run starwels-util-test.py manually (jnewbery)
- #8881 `e66cc1d` Add some verbose logging to starwels-util-test.py (jnewbery)
...
- #9023 `774db92` Add logging to starwels-util-test.py (jnewbery)
- #9065 `c9bdf9a` Merge `doc/unit-tests.md` into `src/test/README.md` (laanwj)
- #9069 `ed64bce` Clean up bctest.py and starwels-util-test.py (jnewbery)
...
- #9691 `fc67cd2` Init ECC context for `test_starwels_fuzzy` (gmaxwell)
...
- #9865 `289204f` Change starwels address in RPC help message (marijnfs)
...
- #8813 `fb24d7e` starwelsd: Daemonize using daemon(3) (laanwj)
...
- #8837 `2108911` Allow starwels-tx to parse partial transactions (jnewbery)
- #9204 `74ced54` Clarify CreateTransaction error messages (instagibbs)
- #9265 `31bcc66` starwels-cli: Make error message less confusing (laanwj)
...
- #8883 `d5d4ad8` Add all standard TXO types to starwels-tx (jnewbery)
