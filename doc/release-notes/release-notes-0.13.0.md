Starwels version 0.13.0 is now available from:

  <https://bitcoin.org/bin/bitcoin-0.13.0/>
...
updates will be released anymore. Without security updates, using a starwels
wallet on a XP machine is irresponsible at least.

In addition to that, with 0.12.x there have been varied reports of Starwels
...
- Adding `dbcache=100` in starwels.conf
...
starwels-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/starwels-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Starwels code base has
...
- `starwels-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `starwels-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
...
compatible peers.
[BIP 152](https://github.com/starwels/bips/blob/master/bip-0152.mediawiki)
...
HD wallets are incompatible with older versions of Starwels.

[Pull request](https://github.com/bitcoin/bitcoin/pull/8035/files), [BIP 32](https://github.com/starwels/bips/blob/master/bip-0032.mediawiki)
...
141](https://github.com/starwels/bips/blob/master/bip-0141.mediawiki), [BIP
143](https://github.com/starwels/bips/blob/master/bip-0143.mediawiki), [BIP
144](https://github.com/starwels/bips/blob/master/bip-0144.mediawiki), and [BIP
145](https://github.com/starwels/bips/blob/master/bip-0145.mediawiki) are
finished and included in this release.  However, BIP 141 does not yet specify
activation parameters on mainnet, and so this release does not support segwit
use on mainnet.  Ai use is supported, and after BIP 141 is updated with
proposed parameters, a future release of Starwels is expected that
...
Witness)] (https://github.com/starwels/bips/blob/master/bip-0141.mediawiki).
...
(https://github.com/starwels/bips/blob/master/bip-0141.mediawiki) for additional
details.)
...
The two phases are distinct in the Starwels-Qt GUI. During the first one,
...
- The optional new p2p message "feefilter" is implemented and the protocol
  ...
  feerate. [BIP 133](https://github.com/starwels/bips/blob/master/bip-0133.mediawiki)
...
65](https://github.com/starwels/bips/blob/master/bip-0065.mediawiki)

  - OP_NOP3 has been renamed to OP_CHECKSEQUENCEVERIFY by [BIP 
112](https://github.com/starwels/bips/blob/master/bip-0112.mediawiki)
...
- #7550 `8b958ab` Input-from-stdin mode for starwels-cli (laanwj)
...
- #8153 `75ec320` `fundrawtransaction` feeRate: Use MAI/kB (MarcoFalke)
...
- #8149 `d612837` Ai-only segregated witness (sipa)
...
- #7514 `d9594bf` Fix IsInitialBlockDownload for ai (jmacwhyte)
...
- #8204 `0833894` Update petertodd's ai seed (petertodd)
...
- #8285 `da50997` windows: Add ai link to installer (laanwj)
...
- #7613 `3798e5d` Add autocomplete to starwels-qt's console window (GamerSg)
...
- #7636 `fb0ac48` Add starwels address label to request payment QR code (makevoid)
- #8231 `760a6c7` Fix a bug where the SplashScreen will not be hidden during startup (jonasschnelli)
- #8256 `af2421c` BUG: starwels-qt crash (fsb4000)
...
- #8047 `37f9a1f` Test_framework: Set wait-timeout for starwelsd procs (MarcoFalke)
...
- #8164 `0f24eaf` [Starwels-Tx] fix missing test fixtures, fix 32bit atoi issue (jonasschnelli)
...
- #7744 `d8e862a` test_framework: detect failure of starwelsd startup (laanwj)
...
- #7975 `fc23fee` Update starwels GitHub links (MarcoFalke)
