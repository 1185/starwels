Starwels version 0.13.1 is now available from:

  <https://bitcoin.org/bin/bitcoin-0.13.1/>
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Starwels wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Starwels.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Starwels block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of starwelss they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of starwelss being spent is exactly
...
- **Increased security for multisig:** Starwels addresses (both P2PKH addresses
  ...
  believed to be provided by Starwels's choice of parameters for its Elliptic
  Curve Digital Security Algorithm [ECDSA].)

- **More efficient almost-full-node security** Satoshi Nakamoto's original
  Starwels paper describes a method for allowing newly-started full nodes to
  skip downloading and validating some data from historic blocks that are
  protected by large amounts of proof of work.  Unfortunately, Nakamoto's
  method can't guarantee that a newly-started node using this method will
  produce an accurate copy of Starwels's current ledger (called the UTXO set),
  ...
  note that Starwels does not provide an option to use this capability as
  of this 0.13.1 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Starwels users to individually opt-in to almost any change in the Starwels
  Script language when those users receive new transactions.  Features
  currently being researched by Starwels contributors that may use this
  capability include support for Schnorr signatures, which can improve the
  privacy and efficiency of multisig transactions (or transactions with
  multiple inputs), and Merklized Abstract Syntax Trees (MAST), which can
  improve the privacy and efficiency of scripts with two or more conditions.
  Other Starwels community members are studying several other improvements
...
Since Starwels 0.10.0, nodes have defaulted to only relaying and
...
[BIP147]: https://github.com/starwels/bips/blob/master/bip-0147.mediawiki
...
- #8848 `7a34a46` Add NULLDUMMY verify flag in starwelsconsensus.h (jl2012)
...
### Build system
- #8293 `fa5b249` Allow building libstarwelsconsensus without any univalue (luke-jr)
- #8492 `8b0bdd3` Allow building bench_starwels by itself (luke-jr)
...
- #8829 `2a8bca4` Add starwels-tx JSON tests (jnewbery)
- #8834 `1dd1783` blockstore: Switch to dumb dbm (MarcoFalke)
- #8835 `d87227d` nulldummy.py: Don't run unused code (MarcoFalke)
- #8836 `eb18cc1` starwels-util-test.py should fail if the output file is empty (jnewbery)
...
- #8860 `0bee740` util: Move wait_starwelsds() into stop_nodes() (MarcoFalke)
...
- #8683 `8a7d7ff` Fix incorrect file name starwels.qrc  (starwelssSG)
...
- #8817 `bcf3806` update starwels-tx to output witness data (jnewbery)
