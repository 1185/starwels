...
*Setup instructions for a Gitian build of Starwels using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Starwels
...
- [Building Starwels](#building-starwels)
...
Clone the git repositories for starwels and Gitian.
...
in the starwels repository under 'Fetch and create inputs' to install sources which require
...
To build Starwels (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the starwels repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/starwels/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/starwels/starwels
...
./bin/gbuild --commit starwels=${COMMIT} --url starwels=${URL} ../starwels/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit starwels=${COMMIT} --url starwels=${URL} ../starwels/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit starwels=${COMMIT} --url starwels=${URL} ../starwels/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the starwels git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../starwels/contrib/gitian-descriptors/*|sort|uniq )
...
MAIPATH=/some/root/path/starwels
SIGPATH=/some/root/path/starwels-detached-sigs

./bin/gbuild --url starwels=${MAIPATH},signature=${SIGPATH} ../starwels/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/starwels-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/starwels-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/starwels-osx-build.assert
```
...
[starwels/gitian.sigs](https://github.com/starwels/gitian.sigs/) repository, or if that's not possible create a pull
