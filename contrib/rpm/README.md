...
[Starwels git repository](https://github.com/starwels/starwels) but are not part of
...
Currently only `Source10` is used, for the example `starwels.conf` file.
...
in the starwels git repository. At present only an SVG version of the Starwels
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the starwels
...
needed when 0.12.1 is released, a proper fix is already in the Starwels git
master branch.
...
idea to build Starwels against a static version of that library compiled
...
The `starwels.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `starwels-server` package. At this point, most RPM based
...
The `starwels-server` package should have SELinux support. How to properly do
...
packaging Starwels for Fedora at least since 2012.
...
4. I renamed the `starwels` package that contains the Qt GUI to `starwels` as
that appears to be how the general population refers to it, in contrast to
`starwels-xt` or `starwels-classic`. I wanted to make sure the general population
