...
AC_DEFUN([STARWELS_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [build starwels-qt GUI (default=auto)])],
    [
...
AC_DEFUN([STARWELS_QT_CONFIGURE],[
  ...
  if test "x$use_pkgconfig" = xyes; then
    STARWELS_QT_CHECK([_STARWELS_QT_FIND_LIBS_WITH_PKGCONFIG])
  else
  ...
  if test "x$starwels_cv_static_qt" = xyes; then
    ...
    AC_CACHE_CHECK(for Qt < 5.4, starwels_cv_need_acc_widget,[
  ...
  STARWELS_QT_PATH_PROGS([MOC], [moc-qt5 moc5 moc], $qt_bin_path)
  STARWELS_QT_PATH_PROGS([UIC], [uic-qt5 uic5 uic], $qt_bin_path)
  STARWELS_QT_PATH_PROGS([RCC], [rcc-qt5 rcc5 rcc], $qt_bin_path)
  STARWELS_QT_PATH_PROGS([LRELEASE], [lrelease-qt5 lrelease5 lrelease], $qt_bin_path)
  STARWELS_QT_PATH_PROGS([LUPDATE], [lupdate-qt5 lupdate5 lupdate],$qt_bin_path, yes)
  ...
  AC_MSG_RESULT([$starwels_enable_qt (Qt5)])
...
dnl Requires: INCLUDES must be populated as necessary.
dnl Output: starwels_cv_qt5=yes|no
AC_DEFUN([_STARWELS_QT_CHECK_QT58],[
  AC_CACHE_CHECK(for > Qt 5.7, starwels_cv_qt58,[
  AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
      #include <QtCore/qconfig.h>
      #ifndef QT_VERSION
      #  include <QtCore/qglobal.h>
...
AC_DEFUN([_STARWELS_QT_FIND_STATIC_PLUGINS],[
    if test "x$qt_plugin_path" != x; then
     ...
     m4_ifdef([PKG_CHECK_MODULES],[
       if test x$starwels_cv_qt58 = xno; then
       ...
       if test "x$TARGET_OS" = xwindows; then
         AC_CACHE_CHECK(for Qt >= 5.6, starwels_cv_need_platformsupport,[
           AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
               #include <QtCore/qconfig.h>
               #ifndef QT_VERSION
               #  include <QtCore/qglobal.h>
         ...
         if test "x$starwels_cv_need_platformsupport" = xyes; then
           if test x$starwels_cv_qt58 = xno; then
             ...
             STARWELS_QT_CHECK(AC_CHECK_LIB([${QT_LIB_PREFIX}FontDatabaseSupport],[main],,STARWELS_QT_FAIL(lib$QT_LIB_PREFIXFontDatabaseSupport not found)))
             STARWELS_QT_CHECK(AC_CHECK_LIB([${QT_LIB_PREFIX}EventDispatcherSupport],[main],,STARWELS_QT_FAIL(lib$QT_LIB_PREFIXEventDispatcherSupport not found)))
             STARWELS_QT_CHECK(AC_CHECK_LIB([${QT_LIB_PREFIX}ThemeSupport],[main],,STARWELS_QT_FAIL(lib$QT_LIB_PREFIXThemeSupport not found)))
             STARWELS_QT_CHECK(AC_CHECK_LIB([${QT_LIB_PREFIX}FbSupport],[main],,STARWELS_QT_FAIL(lib$QT_LIB_PREFIXFbSupport not found)))
             STARWELS_QT_CHECK(AC_CHECK_LIB([${QT_LIB_PREFIX}DeviceDiscoverySupport],[main],,STARWELS_QT_FAIL(lib$QT_LIB_PREFIXDeviceDiscoverySupport not found)))
             STARWELS_QT_CHECK(AC_CHECK_LIB([${QT_LIB_PREFIX}AccessibilitySupport],[main],,STARWELS_QT_FAIL(lib$QT_LIB_PREFIXAccessibilitySupport not found)))
...
AC_DEFUN([_STARWELS_QT_FIND_LIBS_WITHOUT_PKGCONFIG],[
  ...
  if test x$starwels_cv_qt58 = xno; then
    ...
    STARWELS_QT_CHECK(AC_SEARCH_LIBS([png_error] ,[qtlibpng png],,AC_MSG_WARN([libpng not found. Assuming qt has it built-in])))
    STARWELS_QT_CHECK(AC_SEARCH_LIBS([pcre2_match_16], [qtpcre2 libqtpcre2],,AC_MSG_WARN([libqtpcre2 not found. Assuming qt has it built-in])))
  fi
  STARWELS_QT_CHECK(AC_SEARCH_LIBS([hb_ot_tags_from_script] ,[qtharfbuzzng qtharfbuzz harfbuzz],,AC_MSG_WARN([libharfbuzz not found. Assuming qt has it built-in or support is disabled])))
  STARWELS_QT_CHECK(AC_CHECK_LIB([${QT_LIB_PREFIX}Core]   ,[main],,STARWELS_QT_FAIL(lib${QT_LIB_PREFIX}Core not found)))
  STARWELS_QT_CHECK(AC_CHECK_LIB([${QT_LIB_PREFIX}Gui]    ,[main],,STARWELS_QT_FAIL(lib${QT_LIB_PREFIX}Gui not found)))
  STARWELS_QT_CHECK(AC_CHECK_LIB([${QT_LIB_PREFIX}Network],[main],,STARWELS_QT_FAIL(lib${QT_LIB_PREFIX}Network not found)))
  STARWELS_QT_CHECK(AC_CHECK_LIB([${QT_LIB_PREFIX}Widgets],[main],,STARWELS_QT_FAIL(lib${QT_LIB_PREFIX}Widgets not found)))
  QT_LIBS="$LIBS"
  LIBS="$TEMP_LIBS"
