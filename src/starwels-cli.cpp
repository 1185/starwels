...
static void SetupCliArgs()
{
    ...
    gArgs.AddArg("-rpcport=<port>", strprintf("Connect to JSON-RPC on <port> (default: %u or ai: %u)", defaultBaseParams->RPCPort(), aiBaseParams->RPCPort()), false, OptionsCategory::OPTIONS);
    ...
    gArgs.AddArg("-rpcwallet=<walletname>", "Send RPC for non-default wallet on RPC server (needs to exactly match corresponding -wallet option passed to starwelsd)", false, OptionsCategory::OPTIONS);
...
static int AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        if (!gArgs.IsArgSet("-version")) {
            strUsage += "\n"
                "Usage:  starwels-cli [options] <command> [params]  Send command to " PACKAGE_NAME "\n"
                "or:     starwels-cli [options] -named <command> [name=value]...  Send command to " PACKAGE_NAME " (with named arguments)\n"
                "or:     starwels-cli [options] help                List commands\n"
                "or:     starwels-cli [options] help <command>      Get help for a command\n";
    ...
    // Check for -ai or -regtest parameter (BaseParams() calls are only valid after this clause)
    try {
...
public:
    ...
    UniValue ProcessReply(const UniValue &batch_in) override
    {
        ...
        result.pushKV("ai", UniValue(batch[ID_BLOCKCHAININFO]["result"]["chain"].get_str() == "test"));
        if (!batch[ID_WALLETINFO].isNull()) {
            result.pushKV("walletversion", batch[ID_WALLETINFO]["result"]["walletversion"]);
            result.pushKV("balance", batch[ID_WALLETINFO]["result"]["balance"]);
...
static UniValue CallRPC(BaseRequestHandler *rh, const std::string& strMethod, const std::vector<std::string>& args)
{
    ...
    if (response.status == 0) {
        ...
        throw CConnectionFailed(strprintf("Could not connect to the server %s:%d%s\n\nMake sure the starwelsd server is running and that you are connecting to the correct RPC port.", host, port, responseErrorMessage));
    } else if (response.status == HTTP_UNAUTHORIZED) {
        if (failedToGetAuthCookie) {
            throw std::runtime_error(strprintf(
                "Could not locate RPC credentials. No authentication cookie could be found, and RPC password is not set.  See -rpcpassword and -stdinrpcpass.  Configuration file: (%s)",
                GetConfigFile(gArgs.GetArg("-conf", STARWELS_CONF_FILENAME)).string().c_str()));
        } else {
