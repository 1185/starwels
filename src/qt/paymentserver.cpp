...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(STARWELS_IPC_PREFIX, Qt::CaseInsensitive)) // starwels: URI
        {
            ...
            {
                CStarwelsAddress address(r.address.toStdString());
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith(STARWELS_IPC_PREFIX, Qt::CaseInsensitive)) // starwels: URI
    {
#if QT_VERSION < 0x050000
        QUrl uri(s);
#else
        ...
        else // normal URI
        {
            ...
            {
                CStarwelsAddress address(recipient.address.toStdString());
                if (!address.IsValid()) {
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    for (const std::pair<CScript, CAmount>& sendingTo : sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            addresses.append(QString::fromStdString(CStarwelsAddress(dest).ToString()));
        }
