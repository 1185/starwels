...
const int STARWELS_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString STARWELS_IPC_PREFIX("starwels:");
// BIP70 payment protocol messages
const char* BIP70_MESSAGE_PAYMENTACK = "PaymentACK";
const char* BIP70_MESSAGE_PAYMENTREQUEST = "PaymentRequest";
// BIP71 payment protocol media types
const char* BIP71_MIMETYPE_PAYMENT = "application/starwels-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/starwels-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/starwels-paymentrequest";
...
{
    QString name("StarwelsQt");

    // Append a simple hash of the datadir
    // Note that GetDataDir(true) returns a different path
    // for -ai versus main net
...
void PaymentServer::ipcParseCommandLine(interfaces::Node& node, int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        // If the starwels: URI contains a payment request, we are not able to detect the
        // network as that would require fetching and parsing the payment request.
        // That means clicking such an URI which contains a ai payment request
        // will start a mainnet instance and throw a "wrong network" error.
        if (arg.startsWith(STARWELS_IPC_PREFIX, Qt::CaseInsensitive)) // starwels: URI
        {
            savedPaymentRequests.append(arg);

            SendCoinsRecipient r;
            if (GUIUtil::parseStarwelsURI(arg, &r) && !r.address.isEmpty())
            {
                ...
                if (IsValidDestinationString(r.address.toStdString(), *tempChainParams)) {
                    ...
                    tempChainParams = CreateChainParams(CBaseChainParams::AI);
                    if (IsValidDestinationString(r.address.toStdString(), *tempChainParams)) {
                        node.selectParams(CBaseChainParams::AI);
                    }
                }
            }
        }
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    for (const QString& r : savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(STARWELS_IPC_CONNECT_TIMEOUT))
        {
            delete socket;
            socket = nullptr;
            return false;
        ...
        socket->write(block);
        socket->flush();
        socket->waitForBytesWritten(STARWELS_IPC_CONNECT_TIMEOUT);
    ...
    optionsModel(0)
{
    ...
    // on Mac: sent when you click starwels: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start starwels: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling starwels: URIs and PaymentRequest mime types.
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in starwels: URIs
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("starwels://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'starwels://' is not a valid URI. Use 'starwels:' instead."),
            CClientUIInterface::MSG_ERROR);
    }
    else if (s.startsWith(STARWELS_IPC_PREFIX, Qt::CaseInsensitive)) // starwels: URI
    {
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseStarwelsURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid Starwels address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    for (const std::pair<CScript, CAmount>& sendingTo : sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            // Unauthenticated payment requests to custom starwels addresses are not supported
        ...
        // Starwels amounts are stored as (optional) uint64 in the protobuf messages (see paymentrequest.proto),
        ...
        if (IsDust(txOut, optionsModel->node().getDustRelayFee())) {
            Q_EMIT message(tr("Payment request error"), tr("Requested payment amount of %1 is too small (considered dust).")
                .arg(StarwelsUnits::formatWithUnit(optionsModel->getDisplayUnit(), sendingTo.second)),
                CClientUIInterface::MSG_ERROR);
