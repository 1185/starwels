...
const char * const STARWELS_CONF_FILENAME = "starwels.conf";
const char * const STARWELS_PID_FILENAME = "starwelsd.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "starwelsd -foo=bar
    ...
    /* Special test for -ai and -regtest args, because we
     * don't want to be confused by craziness like "[regtest] ai=1"
     */
...
ArgsManager::ArgsManager() :
     ...
     * mainnet were used while running on regtest/ai (or vice-versa).
     * Setting them as section_only_args ensures that sharing a config file
     * between mainnet and regtest/ai won't cause problems due to these
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Starwels
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Starwels
    // Mac: ~/Library/Application Support/Starwels
    // Unix: ~/.starwels
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Starwels";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Starwels";
#else
    // Unix
    return pathRet / ".starwels";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", STARWELS_CONF_FILENAME);
...
std::string ArgsManager::GetChainName() const
{
    ...
    bool fTestNet = ArgsManagerHelper::GetNetBoolArg(*this, "-ai");
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", STARWELS_PID_FILENAME)));
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure Starwels copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Starwels") == std::string::npos) {
